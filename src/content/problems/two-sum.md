...
왜냐하면, `index 0`에 위치한 숫자는 `2`이고 `index 1`에 위치한 숫자는 `7`이기 때문에 이 두 수를 더하면 `target`인 `9`와 일치하기 때문이다.

배열 안에는 정답이 하나(두 개의 숫자)만 있도록 구성되어 있다고 가정하며, 같은 숫자를 두 번 사용할 수 없다.

## 풀이 1: 모든 경우의 수

먼저 가장 무식할 수도 있지만 직관적인 방법으로 이 문제에 접근해보겠습니다.
...
한가지 주의할 점은 Hash Table에 `target - nums[i]`의 존재 여부를 체크할 때 동일 인덱스를 두 번 쓰지 않도록 신경써줘야 한다는 것입니다.
...
Hash Table의 크기는 배열의 크기와 비례해서 증가하기 때문에 공간 복잡도도 `O(n)` 입니다.
...
인덱스 `i`에 대해서 인덱스 `j`는 항상 `i`보다 이전 위치를 가리키기 때문에, 리턴 순서가 `[i, j]`에서 `[j, i]`로 변경되었음에 유의하세요.
